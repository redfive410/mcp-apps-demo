...
mcp = FastMCP(
    ...
    transport_security=TransportSecuritySettings(
        allowed_hosts=["mcp-apps-demo-4vohcegqpq-uw.a.run.app", "unrestricted-nuptially-gage.ngrok-free.dev", "localhost", "127.0.0.1"],
    ),
)
...
def _tool_meta() -> Dict[str, Any]:
    """Generate metadata for tools with MCP Apps UI resource reference."""
    return {
        "ui": {
            "resourceUri": DEMO_WIDGET.template_uri,
        }
    }
...
async def _handle_read_resource(req: types.ReadResourceRequest) -> types.ServerResult:
    ...
    print(f"[DEBUG] resources/read called for URI: {req.params.uri}")
...
def _reply_with_demo(message: str = "") -> types.CallToolResult:
    ...
    return types.CallToolResult(
        content=content,
        structuredContent={
            "message": message or "Demo value is updated.",
            "counter": demo_value,
        },
    )
